...
final class SwitcherPanel {
    ...
    var onPreviousDisplayRequested: (() -> Void)?
    ...
    func show(spaces: [SpaceInfo], allSpaces: [SpaceInfo], selectedIndex: Int,
              displayName: String = "", displayDisconnected: Bool = false, on screen: NSScreen? = nil)
    {
        self.spaces = spaces
    ...
    /// そのスペースが選択可能かどうか
    /// 別ディスプレイのアプリなしスペースは選択不可
    private func isSelectable(at index: Int) -> Bool {
        let space = spaces[index]
        let focusedDisplay = allSpaces.first(where: \.hasFocus)?.displayIndex
        if space.displayIndex != focusedDisplay, space.apps.isEmpty {
            return false
        }
        return true
    }
    ...
    func commitAndClose() async {
        ...
        // アプリがないスペースの場合: 対象ディスプレイにフォーカスしてから Ctrl+数字
        let focusedDisplay = allSpaces.first(where: \.hasFocus)?.displayIndex
        if focusedDisplay != targetSpace.displayIndex {
            await spaceSwitcher.focusDisplay(displayIndex: targetSpace.displayIndex)
            try? await Task.sleep(nanoseconds: 200_000_000) // 0.2秒待機
        }

        // per-display: 同じディスプレイの通常スペースの中で何番目か
        let targetDisplay = targetSpace.displayIndex
        let desktopNumber = allSpaces
            .filter { !$0.isNativeFullscreen && $0.displayIndex == targetDisplay && $0.index <= targetSpace.index }
            .count
